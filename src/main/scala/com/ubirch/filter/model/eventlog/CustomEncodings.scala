...
trait CustomEncodings[T] extends CustomEncodingsBase {
  ...
  implicit def encodeJValue: MappedEncoding[JValue, String] = MappedEncoding[JValue, String](Option(_).map(EventLogJsonSupport.FromJson(_).toString).getOrElse(""))
  implicit def decodeJValue: MappedEncoding[String, JValue] = MappedEncoding[String, JValue](x => parse(x))
